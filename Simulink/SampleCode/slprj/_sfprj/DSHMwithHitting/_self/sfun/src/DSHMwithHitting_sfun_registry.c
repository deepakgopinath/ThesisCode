...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_DSHMwithHitting_process_check_sum_call( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_DSHMwithHitting_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_DSHMwithHitting_autoinheritance_info( int nlhs, mxArray
  ...
  if (strcmp(machineName, "DSHMwithHitting") == 0) {
    const mxArray *newRhs[3] = { NULL, NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    newRhs[2] = prhs[3];
    return sf_DSHMwithHitting_autoinheritance_info(nlhs,plhs,3,newRhs);
  }
...
unsigned int sf_process_get_third_party_uses_info_call( int nlhs, mxArray *
  plhs[], int nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_DSHMwithHitting_third_party_uses_info( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  char commandName[64];
  char machineName[128];
  if (nrhs < 4) {
  ...
  if (strcmp(machineName, "DSHMwithHitting") == 0) {
    ...
    newRhs[1] = prhs[2];
    newRhs[2] = prhs[3];
    return sf_DSHMwithHitting_third_party_uses_info(nlhs,plhs,3,newRhs);
  }
...
unsigned int sf_process_get_updateBuildInfo_args_info_call( int nlhs, mxArray *
  plhs[], int nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_DSHMwithHitting_updateBuildInfo_args_info( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  char commandName[64];
  char machineName[128];
  if (nrhs < 4) {
  ...
  if (strcmp(machineName, "DSHMwithHitting") == 0) {
    ...
    newRhs[1] = prhs[2];
    newRhs[2] = prhs[3];
    return sf_DSHMwithHitting_updateBuildInfo_args_info(nlhs,plhs,3,newRhs);
  }
...
unsigned int sf_process_get_eml_resolved_functions_info_call( int nlhs, mxArray *
  plhs[], int nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_DSHMwithHitting_get_eml_resolved_functions_info( int
  ...
  if (strcmp(machineName, "DSHMwithHitting") == 0) {
    ...
    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_DSHMwithHitting_get_eml_resolved_functions_info(nlhs,plhs,2,newRhs);
  }
...
extern unsigned int sf_DSHMwithHitting_method_dispatcher(SimStruct *S, unsigned
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"DSHMwithHitting")) {
    return(sf_DSHMwithHitting_method_dispatcher(simstructPtr,chartFileNumber,
...
extern void DSHMwithHitting_initializer(void);
extern void DSHMwithHitting_register_exported_symbols(SimStruct* S);
extern void DSHMwithHitting_debug_initialize(struct SfDebugInstanceStruct*);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  DSHMwithHitting_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfGlobalMdlStartCallCounts == 1) {
    ...
    if (!simModeIsRTWGen) {
      DSHMwithHitting_debug_initialize(sfGlobalDebugInstanceStruct);
    }

    DSHMwithHitting_initializer();
  }
